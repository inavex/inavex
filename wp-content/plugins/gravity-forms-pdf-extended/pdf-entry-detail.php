...
class GFPDFEntryDetail {
    ...
    public static function lead_detail_grid($form, $lead, $allow_display_empty_fields=false){
        ...
        ?>
        <div id="container">
            <h2 id="details"><?php echo $form["title"]?> <!--: <?php _e("Entry # ", "gravityforms") ?> <?php echo $lead["id"] ?> --></h2>

            <?php
            if($allow_display_empty_fields){
                ?>
                <div><input type="checkbox" id="gentry_display_empty_fields" <?php echo $display_empty_fields ? "checked='checked'" : "" ?> onclick="ToggleShowEmptyFields();"/>&nbsp;&nbsp;<label for="gentry_display_empty_fields"><?php _e("show empty fields", "gravityforms") ?></label></div>
            <?php
            }
            ?>
                ...
                switch(RGFormsModel::get_input_type($field)){
                    case "section" :
                        if(!GFCommon::is_section_empty($field, $form, $lead) || $display_empty_fields){
                            ...
                            ?>
                            <h2 class="entry-view-section-break<?php echo $is_last ? " lastrow" : ""?>"><?php echo esc_html(GFCommon::get_label($field))?></h2>
                    ...
                    case "signature":
                        ...
                        $public_folder = site_url() .  "/wp-content/uploads/gravity_forms/signatures/";
                        $server_folder = ABSPATH . 'wp-content/uploads/gravity_forms/signatures/';
                        $display_value = '<img src="'. $public_folder.$value .'" alt="Signature" width="100" height="60" />';
                        ...
                        {
                            $content = '<tr class="entry-view-field-value' . $last_row . $even . '"><td>' .  esc_html(GFCommon::get_label($field)) . '</td> <td>' . $display_value . '</td></tr>	';
                    ...
                    default:
                        ...
                        if($display_empty_fields || !empty($display_value) || $display_value === "0"){
                            ...
                            $content = '<tr class="entry-view-field-value' . $last_row . $even . '"><td class="ss"> ' .  esc_html(GFCommon::get_label($field)) . ':&nbsp;</td><td class="tt">' . $display_value . '</td></tr>';
    ...
    public static function product_table($form, $lead)
    {
        ...
        if(!empty($products["products"])){
            ...
            <h2 class="entry-view-field-name"><?php echo apply_filters("gform_order_label_{$form["id"]}", apply_filters("gform_order_label", __("Order", "gravityforms"), $form["id"]), $form["id"]) ?></h2>
    ...
    public static function lead_detail_grid_array($form, $lead, $allow_display_empty_fields=false){
        ...
        foreach($form["fields"] as $field){
            ...
            switch(RGFormsModel::get_input_type($field)){
                ...
                case "signature":
                    $value = RGFormsModel::get_lead_field_value($lead, $field);
                    $http_folder = site_url() .  "/wp-content/uploads/gravity_forms/signatures/";
                    $folder = ABSPATH . "/wp-content/uploads/gravity_forms/signatures/";

                    if(file_exists($folder.$value) !== false)
                    {
                        $form_array['signature'][] = '<img src="'. $http_folder.$value .'" alt="Signature" width="100" height="60" />';
                    }

...
class WPCustomerReviews {
    ...
    function make_p_obj() {
        ...
        if ($_POST['yandexSync'] != '1') {
            ...
            $siteurl = 'http://maps.yandex.ru/org/1130154775/';
            $site = file_get_contents($siteurl);

            $document = phpQuery::newDocument($site);
            $reviews = array();

            $hentry = $document->find('.b-business-review-item');
            ...
            foreach ($hentry as $el) {
                ...
                $pq->html();
                $reviews[] = array(
                  'content' => $pq->find('.b-business-review-item__text')->html(),
                  'name' => $pq->find('.b-business-review-item__author')->html()
                );
            }
            ...
            foreach ($reviews as $review) {
                ...
                $text = $review["content"];
                $textP = trim(strip_tags($review["content"]));
                $name = $review["name"];
                $query = "
                    SELECT `review_text`
                    FROM `inavex_wpcreviews`
                    WHERE `review_text` = '".$textP."'";
                global $wpdb;
                $result = $wpdb->get_results($query);

                if (!$result) {
                    $query = $wpdb->prepare("
                         INSERT INTO inavex_wpcreviews (reviewer_name, review_text, review_rating, page_id, date_time)
                         VALUES (%s, %s, %s, %s, %s)", $name, trim(strip_tags($text)), 5, 430, date("Y-m-d H:i:s"));
        ...
        if (count($reviews) == 0) {
            ...
            foreach ($reviews as $review) {
                ...
                $name_block = '' .
                    ...
                    '<abbr title="' . $this->iso8601(strtotime($review->date_time)) . '" class="dtreviewed">' . date("d. m. Y", strtotime($review->date_time)) . '</abbr>&nbsp;' .
                    ...
                    '<a href="/obratnaya-svyaz/" class="url value-title">Все отзывы пользователя ' . $review->reviewer_name . '</a>' .
                    '</span>' .
                ...
                if ($this->options['hreview_type'] == 'product') {
                    ...
                    $reviews_content .= '
                        <div class="hreview" id="hreview-' . $review->id . '">
                        <a href="/obratnaya-svyaz/" class="permalink" style="display:none;">Отзывы о нас</a>
                            <' . $title_tag . ' class="summary ' . $hidesummary . '">' . $review->review_title . '</' . $title_tag . '>
                            <div class="wpcr_fl wpcr_sc">
                                <abbr class="rating" title="' . $review->review_rating . '"></abbr>
                                <div class="wpcr_rating">
                            ...
                            ' . $name_block . '
                            <div class="wpcr_clear wpcr_spacing1"></div>
                            <span class="item vcard" id="hreview-wpcr-hcard-for-' . $review->id . '">
    ...
    function show_reviews_form() {
        ...
        if ($this->options['ask_fields']['fname'] == 1) {
            ...
            $fields .= '<tr><td><label for="' . $rand_prefixes[0] . '-fname" class="comment-field">Ваше имя: ' . $req . '</label></td><td><input class="text-input" type="text" id="' . $rand_prefixes[0] . '-fname" name="' . $rand_prefixes[0] . '-fname" value="' . $this->p->fname . '" /></td></tr>';
        }
        if ($this->options['ask_fields']['femail'] == 1) {
            ...
            $fields .= '<tr><td><label for="' . $rand_prefixes[1] . '-femail" class="comment-field">Ваш Email: ' . $req . '</label></td><td><input class="text-input" type="text" id="' . $rand_prefixes[1] . '-femail" name="' . $rand_prefixes[1] . '-femail" value="' . $this->p->femail . '" /></td></tr>';
        }
        if ($this->options['ask_fields']['fwebsite'] == 1) {
            ...
            $fields .= '<tr><td><label for="' . $rand_prefixes[2] . '-fwebsite" class="comment-field">Ваш Website: ' . $req . '</label></td><td><input class="text-input" type="text" id="' . $rand_prefixes[2] . '-fwebsite" name="' . $rand_prefixes[2] . '-fwebsite" value="' . $this->p->fwebsite . '" /></td></tr>';
        }
        if ($this->options['ask_fields']['ftitle'] == 1) {
            ...
            $fields .= '<tr><td><label for="' . $rand_prefixes[3] . '-ftitle" class="comment-field">Заголовок сообщения: ' . $req . '</label></td><td><input class="text-input" type="text" id="' . $rand_prefixes[3] . '-ftitle" name="' . $rand_prefixes[3] . '-ftitle" maxlength="150" value="' . $this->p->ftitle . '" /></td></tr>';
        }
                    ...
                    <form class="wpcrcform" id="wpcr_commentform" method="post" action="javascript:void(0);">
                        <div id="wpcr_div_2">
                            ...
                            <table id="wpcr_table_2">
                                <tbody>
                                    <tr><td colspan="2"><div id="wpcr_postcomment">Здесь вы можете оставить отзыв о нашей работе.</div></td></tr>
                                    ' . $fields;

        $out2 = '   
            <tr>
                <td><label class="comment-field">Ваша оценка:</label></td>
                <td><div class="wpcr_rating">' . $this->output_rating(0, true) . '</div></td>
            </tr>';

        $out3 = '
                            <tr><td colspan="2"><label for="' . $rand_prefixes[5] . '-ftext" class="comment-field">Текст отзыва:</label></td></tr>
                            ...
                            <tr>
                                <td colspan="2" id="wpcr_check_confirm">
                                    ...
                                    <div class="wpcr_fl"><input type="checkbox" name="' . $rand_prefixes[7] . '-fconfirm2" id="fconfirm2" value="1" /></div><div class="wpcr_fl" style="margin:-2px 0px 0px 5px"><label for="fconfirm2">Отметьте это поле: (Защита от Спам-ботов)</label></div>
                                    <div class="wpcr_clear"></div>
    ...
    function add_review($pageID) {
        ...
        if ($this->p->femail != '' && $this->options['ask_fields']['femail'] == 1) {
            if (!preg_match('/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/', $this->p->femail)) {
                $errors .= 'Адрес электронной почты указан не корректно.<br />';
            }
        }
        ...
        if (strlen(trim($this->p->ftext)) < 5) {
            $errors .= 'Текст сообщение не может быть меньше 5 символов.<br />';
        }
        ...
        @wp_mail(get_bloginfo('admin_email'), "Вы получили новый отзыв на сайте Автоэкспертизы " . date('m/d/Y h:i'), " \n\nВы должны зайти в административную панель Вашего сайта и опубликовать его.\n\n{$admin_link}");

        /* returns false for no error */
        return array(false, '<div>Спасибо за Ваш отзыв,  он будет опубликован в ближайшее время.</div>');
    }

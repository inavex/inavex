...
function valwpcrform_2(newid,oldid,err) {
    ...
    if (newid === 'femail' && jQuery("#"+oldid).val() !== "") {
        ...
        if (/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/.test(myval) == false) {
            err.push("Адрес электронной почты указан не корректно.");
        }
    }
    ...
    if (newid === "ftext" && jQuery("#"+oldid).val().length < 5) {
        err.push("Текст отзыва не может содержать меньше 5 символов.");
    }
    if (newid === "fconfirm2" && jQuery("#fconfirm2").is(":checked") === false) {
        err.push("Не отмеченно поле защиты от Спам-ботов.");
    }
    if (newid === "fconfirm1" && jQuery("#fconfirm1").is(":checked") ) {
        err.push("Не отмеченно поле защиты от Спам-ботов. Code 2.");
    }
    if (newid === "fconfirm3" && jQuery("#fconfirm3").is(":checked") ) {
        err.push("Не отмеченно поле защиты от Спам-ботов. Code 3.");
    }
...
function valwpcrform() {	
    ...
    if (frating < 1 || frating > 5) {
        err.push("Пожалуйста, поставьте Вашу оценку, выбрав звезду от 1 до 5.");
    }
...
function wpcr_showform() {
    ...
    if (wpcr_old_btn_val == 'Скрыть форму') {
        wpcr_old_btn_val = jQuery("#wpcr_button_1").html();
        jQuery("#wpcr_button_1").html('Скрыть форму');
    } else {
        jQuery("#wpcr_button_1").html(wpcr_old_btn_val);
        wpcr_old_btn_val = 'Скрыть форму';
    }
    jQuery("#wpcr_table_2").find("input:text:visible:first").focus();
}

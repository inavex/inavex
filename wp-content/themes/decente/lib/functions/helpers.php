...
function frameshift_nonce( $expires=86400, $str ) {
    return sha1( date( 'Y-m-d', ceil( time() / $expires ) * $expires ) . $str );
}

add_filter( 'tuc_request_update_query_args-' . FRAMESHIFT_DOMAIN, 'tuc_request_update_query_args' );

function tuc_request_update_query_args( $args ) {

	$args['theme'] = FRAMESHIFT_DOMAIN;
	$args['token'] = frameshift_nonce( 86400, FRAMESHIFT_DOMAIN );
...
$example_update_checker = new ThemeUpdateChecker(
	FRAMESHIFT_DOMAIN,
	'http://update.themeshift.com/api.php'
);